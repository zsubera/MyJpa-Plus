...
/**
 * Shared predicate construction logic for components that build JPA
 * {@link Predicate} instances eagerly (rather than via the deferred
 * {@link ConditionNode} tree).
 * <p>
 * Used by {@link SubQuerySpec} (immediate construction) and
 * {@link com.zsubera.jpa.update.AbstractBulkOperationSpec AbstractBulkOperationSpec}
 * (deferred {@code BiFunction} wrapping), eliminating ~200 lines of
 * duplicated condition logic across the two classes.
 * <p>
 * All methods take a {@link Path}, the resolved field name, the comparison
 * value(s), and a {@link CriteriaBuilder}, and return a fully constructed
 * {@link Predicate}. Null validation and range validation of parameters
 * is the caller's responsibility.
 * <p>
 * Internal utility — not intended for direct use by application code.
 */
public final class PredicateHelper {
    ...
    @SuppressWarnings({"unchecked", "rawtypes"})
    public static Predicate between(Path<?> path, String fieldName, Comparable<?> start, Comparable<?> end, CriteriaBuilder cb) {
        return cb.between((Expression) path.get(fieldName), (Comparable) start, (Comparable) end);
    }

    @SuppressWarnings({"unchecked", "rawtypes"})
    public static Predicate notBetween(Path<?> path, String fieldName, Comparable<?> start, Comparable<?> end, CriteriaBuilder cb) {
