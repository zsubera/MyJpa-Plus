...
public final class FunctionWhitelist {
    ...
    /** 安全函数名正则：仅允许字母和下划线，防止 SQL 注入。 */
    private static final Pattern SAFE_FUNCTION_NAME_PATTERN = Pattern.compile("^[A-Z][A-Z0-9_]*$");
    ...
    public static void addSafeFunctionNames(Collection<String> functionNames) {
        ...
        for (String name : functionNames) {
            if (name != null && !name.isEmpty()) {
                ...
                if (!SAFE_FUNCTION_NAME_PATTERN.matcher(upper).matches()) {
                    throw new IllegalArgumentException(
                        "Invalid function name: '" + name + "'. Function names must contain only "
                            + "alphanumeric characters and underscores, and must start with a letter. "
                            + "This prevents SQL injection through whitelisted function names.");
                }
                EXTRA_SAFE_FUNCTION_NAMES.add(upper);
            }
        }
    ...
    public static void addBooleanFunctionNames(Collection<String> functionNames) {
        ...
        for (String name : functionNames) {
            if (name != null && !name.isEmpty()) {
                ...
                EXTRA_BOOLEAN_FUNCTION_NAMES.add(upper);
            }
        }
        synchronized (FunctionWhitelist.class) {
            freezeExtraFunctionNames();
