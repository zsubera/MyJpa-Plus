 ...
 *
 * <pre>{@code
 * qs.join(User::getRoles, roleJoin -> roleJoin.or(or -> or.eq(Role::getName, "ADMIN"), or -> or.eq(Role::getName, "MODERATOR"))
...
public class OrJoinGroup<T, J> implements ConditionBuilder<J, OrJoinGroup<T, J>> {
    ...
    OrJoinGroup(QuerySpec<T> root, ConditionNode.JoinNode joinNode, ConditionNode.OrNode orNode) {
        ...
        this.conditions = orNode.nodes;
    }

    OrJoinGroup(QuerySpec<T> root, ConditionNode.JoinNode joinNode, ConditionNode.AndNode andNode) {
        if (root == null)
            throw new IllegalArgumentException("root must not be null");
        if (joinNode == null)
            throw new IllegalArgumentException("joinNode must not be null");
        if (andNode == null)
            throw new IllegalArgumentException("andNode must not be null");
        this.conditions = andNode.nodes;
    }
