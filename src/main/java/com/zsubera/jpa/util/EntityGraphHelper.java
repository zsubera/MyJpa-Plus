...
/**
 * Helper for building and applying JPA {@link EntityGraph} fetch strategies
 * dynamically with {@link QuerySpec}.
 * <p>
 * JPA {@link EntityGraph}s allow declarative specification of which associations
 * to fetch eagerly at query time, providing an alternative to {@code FETCH JOIN}
 * with support for reuse across queries.
 * <p>
 * Example:
 * <pre>{@code
...
public final class EntityGraphHelper<T> {
    ...
    /**
     * Sets the graph type to LOAD (hints JPA to load the specified attributes eagerly
     * in addition to any attributes that are already eagerly loaded).
     */
    ...
    /**
     * Sets the graph type to FETCH (only the specified attributes are fetched eagerly;
     * all others are loaded lazily). This is the default.
     */
    ...
    /**
     * Adds a single attribute path to the entity graph.
     * Use dot notation for nested paths: {@code "roles.permissions"}.
     *
    ...
    public <R> TypedQuery<R> apply(TypedQuery<R> query, EntityManager em) {
        ...
        if (log.isDebugEnabled()) {
            log.debug("Applied {} entity graph with {} attributes to query",
                    getHintName(), attributePaths.keySet());
        }
        return query;
    }

    /**
     * Converts this entity graph into a map of JPA query hints for use with
     * repository find methods that accept hints (e.g., via {@code @QueryHints}).
     *
    ...
    public EntityGraph<T> buildGraph(EntityManager em) {
        ...
        if (log.isDebugEnabled()) {
            log.debug("Built {} entity graph for {}: {}", getHintName(), entityClass.getSimpleName(), attributePaths.keySet());
        }
        return graph;
    }
     ...
     * <ul>
     *   <li>FETCH graph: {@code jakarta.persistence.fetchgraph}</li>
     *   <li>LOAD graph: {@code jakarta.persistence.loadgraph}</li>
     * </ul>
     */
