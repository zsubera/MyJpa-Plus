...
public final class SoftDeleteBulkExecutor {
    ...
    public static <T> int softDeleteAll(EntityManager em, Class<T> entityClass, boolean allowUnconditional,
        int maxRows) {
        if (em == null) throw new IllegalArgumentException("em must not be null");
        if (entityClass == null) throw new IllegalArgumentException("entityClass must not be null");
        if (!allowUnconditional) throw new IllegalStateException(
            "softDeleteAll without conditions is dangerous. Pass allowUnconditional=true to confirm.");
        if (!TransactionSynchronizationManager.isActualTransactionActive()) throw new MyJpaPlusException(
            "softDeleteAll requires an active transaction. Ensure the calling method is annotated with @Transactional.");
        if (log.isWarnEnabled()) log.warn("AUDIT: Executing unconditional soft DELETE on {} — this will affect ALL rows! Call stack: {}",
            entityClass.getSimpleName(), AuditUtils.getCallStack());

        String fieldName = SoftDeleteHelper.findSoftDeleteField(entityClass);
        if (fieldName == null) throw new IllegalArgumentException("Entity " + entityClass.getSimpleName() + " has no @SoftDelete field");
        if (hasVersionField(entityClass)) log.warn("AUDIT: Entity {} has @Version field. softDeleteAll() bypasses optimistic lock checking. "
        ...
        if (field == null) throw new IllegalArgumentException("Cannot resolve @SoftDelete field: " + fieldName);
        ...
        SoftDeleteHelper.ResolvedDeletedValue resolved = SoftDeleteHelper.resolveDeletedValue(entityClass, field, annotation);
        ...
        if (maxRows > 0) {
            ...
            if (resolved.booleanField()) probeQuery.setParameter(1, Boolean.FALSE);
            else probeQuery.setParameter(1, resolved.dbValue());
            ...
            if (probeResults.size() > maxRows) {
                ...
                if (resolved.booleanField()) countQuery.setParameter(1, Boolean.FALSE);
                else countQuery.setParameter(1, resolved.dbValue());
                long rowCount = ((Number) countQuery.getSingleResult()).longValue();
                if (rowCount > maxRows) throw new IllegalStateException(
        ...
        if (resolved.booleanField()) {
            String setClause = escapedColumn + " = ?1" + (timestampColumn != null ? ", " + timestampColumn + " = CURRENT_TIMESTAMP" : "");
            updated = em.createNativeQuery("UPDATE " + escapedTable + " SET " + setClause + " WHERE " + escapedColumn + " = ?2 OR " + escapedColumn + " IS NULL")
                .setParameter(1, Boolean.TRUE).setParameter(2, Boolean.FALSE).executeUpdate();
        } else {
            String setClause = escapedColumn + " = ?1" + (timestampColumn != null ? ", " + timestampColumn + " = CURRENT_TIMESTAMP" : "");
            updated = em.createNativeQuery("UPDATE " + escapedTable + " SET " + setClause + " WHERE " + escapedColumn + " != ?1 OR " + escapedColumn + " IS NULL")
                .setParameter(1, resolved.dbValue()).executeUpdate();
        }

        if (maxRows > 0 && updated > maxRows) throw new IllegalStateException("softDeleteAll affected " + updated
    ...
    public static <T, ID> int softDeleteByIds(EntityManager em, Class<T> entityClass, List<ID> ids) {
        ...
        if (ids.size() > hardLimit) throw new IllegalArgumentException("ID list size (" + ids.size() + ") exceeds the hard limit (" + hardLimit + "). "
    ...
    public static <T> int softDeleteAllUsingCriteriaUpdate(EntityManager em, Class<T> entityClass,
        ...
        String fieldName = SoftDeleteHelper.findSoftDeleteField(entityClass);
        if (fieldName == null) throw new IllegalArgumentException("Entity " + entityClass.getSimpleName() + " has no @SoftDelete field");
        if (log.isWarnEnabled()) log.warn("AUDIT: Executing CriteriaUpdate soft DELETE on {} — this will affect ALL rows! Call stack: {}",
            entityClass.getSimpleName(), AuditUtils.getCallStack());
    ...
    public static <T, ID> int softDeleteByIdsUsingEntityManager(EntityManager em, Class<T> entityClass, List<ID> ids) {
        ...
        for (int i = 0; i < ids.size(); i += batchSize) {
            ...
            if (resolved.booleanField()) batchUpdate.set(fieldName, Boolean.TRUE);
            else batchUpdate.set(fieldName, (Comparable) resolved.dbValue());
            if (timestampField != null) batchUpdate.set(annotation.deletedTimestampField(), cb.currentTimestamp());
            batchUpdate.where(batchRoot.get(idFieldName).in(batch));
            total += em.createQuery(batchUpdate).executeUpdate();
        }
        if (total > 0) { em.flush(); publishEvent(entityClass, total); }
        return total;
    }

    static String resolveTimestampColumn(Class<?> entityClass, SoftDelete annotation) {
        if (annotation == null || annotation.deletedTimestampField().isEmpty()) return null;
        return SoftDeleteHelper.validateIdentifier(SoftDeleteHelper.resolveColumnName(entityClass, annotation.deletedTimestampField()));
    }
    ...
    static Field resolveTimestampField(Class<?> entityClass, SoftDelete annotation) {
        ...
        if (f == null) log.warn("SoftDelete deletedTimestampField '{}' not found in {}. Ignoring timestamp.",
