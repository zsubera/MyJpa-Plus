...
final class EntityFieldExtractor<T> {
    ...
    private static final SampledEvictionCache<String, List<String>> ID_COLUMN_CACHE =
        new SampledEvictionCache<>(256, 0.75, 1024);

    /** 缓存 resolveJavaFieldToDbColumn 的结果，避免每次跨实例调用时重复反射扫描类层次 */
    private static final SampledEvictionCache<String, String> JAVA_FIELD_TO_DB_COLUMN_CACHE =
        new SampledEvictionCache<>(1024, 0.75, 1024);
    ...
    private List<EntityFieldValue> extractFieldValues(Object entity, Set<Object> visited) {
        ...
        if (java.util.concurrent.ThreadLocalRandom.current().nextInt(CACHE_CHECK_SAMPLING) == 0) {
            long cacheSize = FIELD_CACHE.estimatedSize();
        ...
        List<Field> allFields = FIELD_CACHE.get(effectiveClass, cls -> {
        ...
        for (Field subField : getAllFields(embeddedValue.getClass())) {
            ...
            if (subField.isAnnotationPresent(jakarta.persistence.Embedded.class)) {
                ...
                if (nestedValue != null) {
                    ...
                    // 不移除 visited 条目，确保同一实例在不同分支中被检测为循环引用
    ...
    private static List<Field> getAllFields(Class<?> clazz) {
        return ALL_FIELDS_CACHE.get(clazz, cls -> {
            List<Field> fields = new ArrayList<>();
            Class<?> current = cls;
        ...
        if (field.getType() == boolean.class || field.getType() == Boolean.class) {
            ...
            if (cachedIs == NO_GETTER_SENTINEL) {
                ...
                try {
                    ...
                    throw new RuntimeException("Getter invocation failed for " + cls.getName() + "#" + isGetterName,
                        cause);
                }
            }
        }
    ...
    boolean isAutoGeneratedId(String fieldName) {
        ...
        boolean result = AUTO_GENERATED_ID_CACHE.get(cacheKey, k -> {
