...
public class MergeSpec<T> {
    ...
    private int doBatchMultiRow(List<T> entities, EntityManager em, int batchSize, DialectStrategy strategy) {
        ...
        List<EntityFieldExtractor.EntityFieldValue> firstFvs = fieldExtractor.extractFieldValues(entities.get(0));
        List<String> effectiveUpdateFields =
            explicitUpdateFields ? updateFields : allNonConflictColumns(firstFvs, conflictSet);
        int total = 0;
        try {
            for (int i = 0; i < entities.size(); i += batchSize) {
                ...
                // 首个实体的字段值已在循环外提取（firstFvs），复用避免重复反射。
                ...
                for (int j = i; j < end; j++) {
                    ...
                    List<EntityFieldExtractor.EntityFieldValue> allFvs =
                        (i == 0 && j == 0) ? firstFvs : fieldExtractor.extractFieldValues(entity);
