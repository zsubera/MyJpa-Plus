...
public class OptimisticLockRetryAdvisor {
    ...
    public Object retryOnOptimisticLock(ProceedingJoinPoint pjp) throws Throwable {
        ...
        RetryOnOptimisticLock annotation = ANNOTATION_CACHE.computeIfAbsent(method, m -> {
            ...
            if (found == null) {
                ...
                try {
                    Method targetMethod = pjp.getTarget().getClass().getMethod(m.getName(), m.getParameterTypes());
     ...
     *
     * <p>
     * 所有尝试均使用 {@code REQUIRES_NEW}（挂起现有事务），确保：
     * <ul>
     * <li>失败时仅回滚新事务，不污染调用方事务状态</li>
     * <li>重试时获得干净的持久化上下文（L1 缓存已清除）</li>
     * <li>避免调用方事务被意外标记为 rollback-only 导致 UnexpectedRollbackException</li>
     * </ul>
     */
            ...
            try {
                ...
                if (!status.isCompleted()) {
                    try {
                        ...
                        log.warn("Failed to rollback transaction after exception: {}", rollbackEx.getMessage());
                    }
                }
                throw t;
            }
