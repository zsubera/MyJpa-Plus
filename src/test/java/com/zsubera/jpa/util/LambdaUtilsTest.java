...
class LambdaUtilsTest {
    ...
    void shouldEvictCacheWhenSizeExceedsMax() throws Exception {
        ...
        try {
            ...
            // Fill cache beyond maxSize via put
            for (int i = 0; i < 30; i++) {
                cache.put("class" + i + "#method" + i, "prop" + i);
            }

            // Caffeine's eviction is lazy — estimatedSize() may not reflect immediate eviction.
            // Verify cache doesn't grow unbounded after many puts.
            assertTrue(cache.size() <= 50, "Cache should not grow unbounded, size is " + cache.size());
        } finally {
