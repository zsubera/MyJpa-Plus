...
class MySQLSoftDeleteIntegrationTest {
    ...
    @BeforeEach
    void setUp() {
        SoftDeleteBulkExecutor.softDeleteAll(em, SoftDeleteEntity.class, true);
        em.flush();
    }
    ...
    void testIsDeletedReturnsOnlySoftDeletedRecords() {
        ...
        List<SoftDeleteEntity> result = SoftDeleteContext.withIgnore(() -> repository.findAll(spec));
    ...
    void testSoftDeleteByIdsMarksRecordsAsDeleted() {
        ...
        int count = SoftDeleteBulkExecutor.softDeleteByIds(em, SoftDeleteEntity.class, List.of(entity1.getId()));
        assertEquals(1, count);

        em.clear();

        SoftDeleteEntity found1 = SoftDeleteContext.withIgnore(() -> repository.findById(entity1.getId()).orElse(null));
        assertNotNull(found1);
        assertTrue(found1.getActive(), "Entity should be soft deleted (active=true means deleted)");

        SoftDeleteEntity found2 = SoftDeleteContext.withIgnore(() -> repository.findById(entity2.getId()).orElse(null));
    ...
    void testSoftDeleteAllMarksAllRecordsAsDeleted() {
        ...
        int count = SoftDeleteBulkExecutor.softDeleteAll(em, SoftDeleteEntity.class, true);
