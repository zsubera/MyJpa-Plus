...
class UpdateSpecTest {
  ...
  void testUpdateWithGeCondition() {
    ...
    assertEquals(
        "ge",
        repository.findAll().stream().filter(e -> e.getStatus() == 5).findFirst().get().getName());
  }
  ...
  void testUpdateWithLikeCondition() {
    ...
    em.clear();
    TestEntity updated =
        repository.findAll().stream().filter(e -> "hello".equals(e.getName())).findFirst().get();
  ...
  void testUpdateNoSetClausesThrowsException() {
    UpdateSpec<TestEntity> spec =
        new UpdateSpec<>(TestEntity.class).eq(TestEntity::getName, "test");
  ...
  void testUpdateNotIn() {
    ...
    em.clear();
    TestEntity updated =
        repository.findAll().stream().filter(e -> "notIn".equals(e.getName())).findFirst().get();
