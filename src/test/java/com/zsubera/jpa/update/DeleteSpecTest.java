...
class DeleteSpecTest {
  ...
  void testDeleteEqNullBecomesIsNull() {
    ...
    int count =
        new DeleteSpec<>(TestEntity.class).eq(TestEntity::getName, (String) null).execute(em);
  ...
  void testDeleteNeNullBecomesIsNotNull() {
    ...
    int count =
        new DeleteSpec<>(TestEntity.class).ne(TestEntity::getName, (String) null).execute(em);
  ...
  void testDeleteNotLike() {
    ...
    repository.save(newEntity("world", 1));
    int count =
        new DeleteSpec<>(TestEntity.class).notLike(TestEntity::getName, "%hello%").execute(em);
    assertEquals(1, count);
  ...
  void testDeleteStartsWith() {
    ...
    repository.save(newEntity("world", 1));
    int count =
        new DeleteSpec<>(TestEntity.class).startsWith(TestEntity::getName, "hel").execute(em);
    assertEquals(1, count);
    assertEquals("world", repository.findAll().get(0).getName());
  }
  ...
  void testDeleteEqIgnoreCase() {
    ...
    repository.save(newEntity("WORLD", 1));
    int count =
        new DeleteSpec<>(TestEntity.class).eqIgnoreCase(TestEntity::getName, "HELLO").execute(em);
    assertEquals(1, count);
    assertEquals("WORLD", repository.findAll().get(0).getName());
  }
  ...
  void testDeleteNotBetween() {
    ...
    repository.save(newEntity("c", 10));
    int count =
        new DeleteSpec<>(TestEntity.class).notBetween(TestEntity::getStatus, 3, 7).execute(em);
    assertEquals(2, count);
    assertEquals(1, repository.findAll().size());
  }
  ...
  void testDeleteWhere() {
    ...
    repository.save(newEntity("b", 10));
    int count =
        new DeleteSpec<>(TestEntity.class).where(root -> root.get("status").in(1)).execute(em);
    assertEquals(1, count);
    assertEquals("b", repository.findAll().get(0).getName());
  }
