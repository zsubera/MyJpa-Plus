...
class MyJpaRepositoryTest {
  ...
  @Autowired private MyJpaTestRepository repository;
  ...
  void testCountNotDeletedWithSpec() {
    ...
    long count =
        repository.countNotDeleted(
            (Specification<MyJpaTestEntity>)
                (root, query, cb) -> cb.equal(root.get("name"), "match"));
  ...
  void testFindNotDeletedAllWithSpec() {
    ...
    List<MyJpaTestEntity> result =
        repository.findNotDeletedAll(
            (Specification<MyJpaTestEntity>)
                (root, query, cb) -> cb.equal(root.get("name"), "target"));
    assertEquals(1, result.size());
