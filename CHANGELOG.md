...
- **EncryptionKeyManager 旧密文多密钥切换不可解密** — 从单密钥切换到多密钥配置后，无 `vN:` 前缀的旧密文因版本为 "default" 无法匹配任何条目而抛出不明确的异常。修复：为 "default" 版本提供明确的迁移指引异常消息
- **DialectDetector JDBC URL 检查可被主机名欺骗** — `contains("postgresql")` 匹配整个 URL 字符串，主机名包含关键词时误判方言（如 `jdbc:mysql://postgresql-replica.internal:3306/mydb`）。修复：改为匹配 JDBC 子协议 `:postgresql:` 等
- **CteSpec setParameter() 与内部参数名冲突** — 用户可调用 `setParameter("_cte_0_param_0", value)` 覆盖 `asSafe()` 生成的内部参数值。修复：禁止以 `_cte_` 开头的参数名
